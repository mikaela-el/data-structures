...
var addresses = JSON.parse(fs.readFileSync("m06Address.txt"))
...
async.eachSeries(addresses, function(value, callback) { // does this in order
    ...
    request(apiRequest, function(err, resp, body) {
        if (err) {throw err;}
            ...
            thisGeo.long = tamuGeo['OutputGeocodes'][0]['OutputGeocode']['Longtitude'];
            console.log(thisGeo);
    ...
    fs.writeFileSync('m06Address.json', JSON.stringify(meetingsData)); // writing a file with this info 
    console.log('*** *** *** *** ***');
